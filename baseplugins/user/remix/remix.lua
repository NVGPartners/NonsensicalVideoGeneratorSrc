...
function PostCommand(commandindex, outputresult, errorresult, options, pluginSettings, functions)
    ...
    elseif commandindex == 3+indexoffset then
        if randomSound == "" then
            ...
            functions.runFFmpeg("-r 30 -i \"" .. temp2 .. "\" -i \"" .. temp3 .. "\" -i \"" .. temp4 .. "\" -filter_complex \"[0:v][0:a][0:v][0:a][2:v][2:a][0:v][0:a][0:v][0:a][2:v][2:a][0:v][0:a][0:v][0:a][0:v][0:a][2:v][2:a][0:v][0:a][2:v][2:a][0:v][0:a][2:v][2:a][0:v][0:a][0:v][0:a][1:v][1:a][1:v][1:a][1:v][1:a][1:v][1:a][1:v][1:a][1:v][1:a][2:v][2:a][1:v][1:a][1:v][1:a][2:v][2:a][1:v][1:a][1:v][1:a][1:v][1:a][1:v][1:a][1:v][1:a][1:v][1:a][2:v][2:a][1:v][1:a][1:v][1:a][1:v][1:a][1:v][1:a][1:v][1:a][1:v][1:a][2:v][2:a][1:v][1:a][1:v][1:a]concat=n=42:v=1:a=1[remixv][remixa];[remixa]volume=volume=1,aresample=async=1000[ogaudio]\" -map \"[remixv]\" -map \"[ogaudio]\" -vcodec libx264 -crf 28 -preset ultrafast -ac 2 -c:a aac -b:a 160k -reset_timestamps 1 -shortest -fflags +genpts -fps_mode vfr -y " .. options.outputVideo)
        else
