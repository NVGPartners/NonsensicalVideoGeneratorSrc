...
namespace NonsensicalVideoGenerator
{
                                    ...
                                    if (pair.Key.Type == DataType.String)
                                    {
                                        switch (pair.Key.String)
                                        {
                                            case "colorTable":
                                                ...
                                                break;
                                            case "musicAttributions":
                                                if (pair.Value.Type == DataType.Table)
                                                {
                                                    foreach (TablePair musicPair in pair.Value.Table.Pairs)
                                                    {
                                                        if (musicPair.Key.Type == DataType.String && musicPair.Value.Type == DataType.String)
                                                        {
                                                            themeMusicAttributions.Add(musicPair.Key.String);
                                                        }
                                                    }
                                                }
                                                break;
    ...
    public static class PluginHandler
    {
        ...
        {
            // .\plugins\user\addon\ should be the most amount of transcending.
            if (path.Split(Path.DirectorySeparatorChar).Length > 4)
                return;
        ...
        public static bool CreatePlugin(string filename, string prettyname, int templateType, out string file)
        {
            try
            {
                ...
                fileContents = fileContents.Replace("%filenamenoext%", filename);
        ...
        public static void OnWorkshopItemUpdated(SubmitItemUpdateResult_t param, bool bIOFailure)
        {
            ...
            publishing = false;
            // Open workshop URL in steam client
            if (publishPlugin != null)
            {
